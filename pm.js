...
const particleImage =
     "https://play-lh.googleusercontent.com/UbZMxdLeK9SPe1NSuecE_Kp23KVZr3JAmwI-x_ETwzNtsFsIdSRez_L9QVEeuwlRpg",
...
let spherePoints, cubePoints, roketPoints, spacemanPoints;

var particles = new THREE.Geometry(),
     sphereParticles = new THREE.Geometry(),
     cubeParticles= new THREE.Geometry(),
     roketParticles = new THREE.Geometry(),
...
objLoader.load("CartoonRocket.obj", function (object) {
     ...
     if (child instanceof THREE.Mesh) {
          ...
          roketPoints = THREE.GeometryUtils.randomPointsInBufferGeometry(child.geometry, particleCount);
          createVertices(roketParticles, roketPoints, yOffset, 2);
     }
     });
});
...
{
     stats.begin();
     particleSystem.rotation.y += aniamtionVars.speed;
...
triggers[2].addEventListener('click', toRoket);
triggers[3].addEventListener('click', toSpaceman);

function handleTriggers(disabled)
{
